#include <jwt-cpp/jwt.h>
#include <iostream>
#include <string>

#include <mbedtls/x509.h>
#include <mbedtls/pem.h>

#ifdef _DEBUG
#pragma comment(lib, "mbedcryptod.lib")
#pragma comment(lib, "mbedx509d.lib")
#else
#pragma comment(lib, "mbedcrypto.lib")
#pragma comment(lib, "mbedx509.lib")
#endif

std::string rsa_priv_key = R"(-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----)";
std::string rsa_pub_key = R"(-----BEGIN PUBLIC KEY-----

YwIDAQAB
-----END PUBLIC KEY-----)";
std::string rsa_pub_key_invalid = R"(-----BEGIN PUBLIC KEY-----

YwIDAQAB
-----END PUBLIC KEY-----)";
std::string rsa512_priv_key = R"(-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----)";
std::string rsa512_pub_key = R"(-----BEGIN PUBLIC KEY-----
MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDdlatRjRjogo3WojgGHFHYLugd
UWAY9iR3fy4arWNA1KoS8kVw33cJibXr8bvwUAUparCwlvdbH6dvEOfou0/gCFQs
HUfQrSDv+MuSUMAe8jzKE4qW+jK+xQU9a03GUnKHkkle+Q0pX/g6jXZ7r1/xAK5D
o2kQ+X5xK9cipRgEKwIDAQAB
-----END PUBLIC KEY-----)";
std::string rsa512_pub_key_invalid = R"(-----BEGIN PUBLIC KEY-----

YwIDAQAB
-----END PUBLIC KEY-----)";
std::string ecdsa_priv_key = R"(-----BEGIN PRIVATE KEY-----
MIGHAgEAMBMGByqGSM49AgEGCCqGSM49AwEHBG0wawIBAQQgPGJGAm4X1fvBuC1z
SpO/4Izx6PXfNMaiKaS5RUkFqEGhRANCAARCBvmeksd3QGTrVs2eMrrfa7CYF+sX
sjyGg+Bo5mPKGH4Gs8M7oIvoP9pb/I85tdebtKlmiCZHAZE5w4DfJSV6
-----END PRIVATE KEY-----)";
std::string ecdsa_pub_key = R"(-----BEGIN PUBLIC KEY-----
MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEQgb5npLHd0Bk61bNnjK632uwmBfr
F7I8hoPgaOZjyhh+BrPDO6CL6D/aW/yPObXXm7SpZogmRwGROcOA3yUleg==
-----END PUBLIC KEY-----)";
std::string ecdsa_pub_key_invalid = R"(-----BEGIN PUBLIC KEY-----
MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEoBUyo8CQAFPeYPvv78ylh5MwFZjT
CLQeb042TjiMJxG+9DLFmRSMlBQ9T/RsLLc+PmpB1+7yPAR+oR5gZn3kJQ==
-----END PUBLIC KEY-----)";

int main()
{
	if(1)
	{
		std::string token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXUyJ9.eyJpc3MiOiJhdXRoMCJ9.AbIJTDMFc7yUa5MhvcP03nJPyCPzZtQcGEp-zWfOkEE";
		auto verify = jwt::verify().allow_algorithm(jwt::algorithm::hs256("secret"));
		auto decoded = jwt::decode(token);

		verify.verify(decoded);

		std::cout << "==================================================" << std::endl;
		std::cout << decoded.get_payload() << std::endl;

		std::cout << (decoded.has_algorithm()) << std::endl;
		std::cout << (decoded.has_type()) << std::endl;
		std::cout << (decoded.has_content_type()) << std::endl;
		std::cout << (decoded.has_key_id()) << std::endl;
		std::cout << (decoded.has_issuer()) << std::endl;
		std::cout << (decoded.has_subject()) << std::endl;
		std::cout << (decoded.has_audience()) << std::endl;
		std::cout << (decoded.has_expires_at()) << std::endl;
		std::cout << (decoded.has_not_before()) << std::endl;
		std::cout << (decoded.has_issued_at()) << std::endl;
		std::cout << (decoded.has_id()) << std::endl;

		std::cout << "HS256 == " << decoded.get_algorithm() << std::endl;
		std::cout << "JWS == " << decoded.get_type() << std::endl;
		std::cout << "auth0 == " << decoded.get_issuer() << std::endl;
		std::cout << "==================================================" << std::endl;
	}

	if (1)
	{
		const std::string token = "eyJhbGciOiJFUzI1NiJ9.eyJpc3MiOiJhdXRoMCJ9.4iVk3-Y0v4RT4_9IaQlp-8dZ_4fsTzIylgrPTDLrEvTHBTyVS3tgPbr2_IZfLETtiKRqCg0aQ5sh9eIsTTwB1g";

		mbedtls_pk_context pk = {0};
		mbedtls_pk_init(&pk);

		mbedtls_pk_parse_public_key(&pk, (const unsigned char*)ecdsa_pub_key.c_str(), ecdsa_pub_key.length() + 1);

		auto verify = jwt::verify().allow_algorithm(jwt::algorithm::es256(mbedtls_pk_ec(pk)));
		auto decoded_token = jwt::decode(token);

		verify.verify(decoded_token);

		mbedtls_pk_free(&pk);
	}

	return 0;
}